    ...
    <div class="items-center block w-full h-auto grow basis-full" id="sidenav-collapse-main">
        <ul class="flex flex-col pl-0 mb-0 list-none">
            ...
            <li class="mt-0.5 w-full">
                <a class="ease-soft-in-out py-2.7 text-size-sm my-0 mx-4 flex items-center whitespace-nowrap px-4 {{ Route::currentRouteName() == 'lecturer.dashboard' ? 'font-semibold text-slate-700 xl:shadow-soft-xl rounded-lg bg-white' : 'font-medium text-slate-500 shadow-none' }} transition-colors dark:text-white dark:opacity-80"
                    href="{{ route('lecturer.dashboard') }}">
                    <div class="stroke-none mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center fill-current p-2.5 text-center text-black {{ Route::currentRouteName() == 'lecturer.dashboard' ? 'shadow-soft-sm bg-gradient-fuchsia' : 'shadow-soft-2xl' }}">
                        ...
                        <svg width="12px" height="12px" viewBox="0 0 40 40" version="1.1" xmlns="http://www.w3.org/2000/svg">
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <rect class="{{ Route::currentRouteName() == 'lecturer.dashboard' ? '' : 'fill-slate-800' }}" fill="#FFFFFF" x="0" y="0" width="40" height="28" rx="2"/>
                                <rect fill="#FFFFFF" x="2" y="2" width="36" height="24" rx="1"/>
                                <rect class="{{ Route::currentRouteName() == 'lecturer.dashboard' ? '' : 'fill-slate-800' }}" x="17" y="28" width="6" height="6"/>
                                <rect class="{{ Route::currentRouteName() == 'lecturer.dashboard' ? '' : 'fill-slate-800' }}" x="10" y="34" width="20" height="3" rx="1"/>
                    ...
                    <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft">Lecturer Dashboard</span>
                </a>
            </li>

            <!-- Roles Switcher (only shown if user has extra capabilities) -->
            @if(auth()->user()->canActAsAdmin() || auth()->user()->canActAsCit() || auth()->user()->canActAsHod() || auth()->user()->canActAsExamOfficer() || auth()->user()->canActAsCoordinator())
