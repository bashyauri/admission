<div x-data="{
    ...
    addMatricUrl(userId) {
        return '/admin/add-matric-number/' + userId;
    },
    
    confirmPaymentUrl(rrr) {
        return '/admin/payment/status/' + rrr;
    },
    
    get allSchoolFees() {
        return @js($schoolFees);
    },
    
    get filteredSchoolFees() {
        let query = this.allSchoolFees;
        ...
        if (this.search) {
            query = query.filter(item => {
                ...
                return (
                    (item.surname + ' ' + item.firstname + ' ' + item.m_name).toLowerCase().includes(searchTerm) ||
                    (item.RRR && item.RRR.toLowerCase().includes(searchTerm)) ||
                    (item.department_name && item.department_name.toLowerCase().includes(searchTerm)) ||
                    (item.matric_no && item.matric_no.toLowerCase().includes(searchTerm)) ||
                    (item.level_name && item.level_name.toLowerCase().includes(searchTerm))
                );
            });
        }
        ...
        if (this.departmentFilter) {
            query = query.filter(item => 
                item.department_name && item.department_name.toLowerCase() === this.departmentFilter.toLowerCase()
            );
        }
        ...
        if (this.levelFilter) {
            query = query.filter(item => 
                item.level_name && item.level_name.toLowerCase() === this.levelFilter.toLowerCase()
            );
        }
        ...
        if (this.statusFilter) {
            query = query.filter(item => 
                item.status && item.status.toLowerCase() === this.statusFilter.toLowerCase()
            );
        }
    ...
    get paginatedSchoolFees() {
        const start = (this.currentPage - 1) * parseInt(this.perPage);
        const end = start + parseInt(this.perPage);
        return this.filteredSchoolFees.slice(start, end);
    },
    
    get totalPages() {
        return Math.ceil(this.filteredSchoolFees.length / parseInt(this.perPage));
    },
        ...
        <div class="relative flex flex-col min-w-0 break-words bg-white border-0 dark:bg-gray-950 dark:shadow-soft-dark-xl shadow-soft-xl rounded-2xl bg-clip-border">
                ...
                <div class="lg:flex">
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            <p class="text-sm text-gray-600 mt-1">Undergraduate students who have paid school fees</p>
                            <div class="flex gap-2 text-sm">
                                <span class="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">
                                    Total: <span x-text="allSchoolFees.length"></span>
                                </span>
                                <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium" x-show="filteredSchoolFees.length < allSchoolFees.length">
                                    Filtered: <span x-text="filteredSchoolFees.length"></span>
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input 
                                type="text" 
                                x-model="search"
                                placeholder="Search by name, RRR, matric no, dept, level..."
            ...
            <div class="flex-auto p-6 px-0 pb-0">
                <div class="overflow-x-auto table-responsive">
                    <table class="table" id="ug-school-fees-table">
                ...
                <div class="p-6 pt-0" x-show="totalPages > 1">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            ...
                            <span x-text="Math.min(currentPage * perPage, filteredSchoolFees.length)"></span> of 
                            <span x-text="filteredSchoolFees.length"></span> results
                        </div>
