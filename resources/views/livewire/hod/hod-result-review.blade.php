                ...
                <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex flex-wrap justify-between items-center gap-4">
                    <div>
                        <h6 class="dark:text-white font-bold text-lg">
                            📋 HOD Result Review & Approvals
                        </h6>
                        <p class="text-sm text-slate-500">
                            @if($inspectingCourse)
                                Reviewing scores for <strong class="text-slate-800">{{ $inspectingCourse->studentCourse->code ?? '' }} - {{ $inspectingCourse->studentCourse->title ?? '' }}</strong>
                            @else
                                Review and approve continuous assessment and examination results submitted by lecturers.
                            @endif
                        </p>
                    </div>
                    ...
                    @if($inspectingCourse)
                        <div class="flex items-center gap-2">
                            ...
                            <button wire:click="openRejectModal" class="px-4 py-2 text-xs font-bold text-white uppercase bg-red-600 rounded-lg shadow hover:bg-red-500 transition">
                                ❌ Return to Lecturer
                            </button>
                            <button wire:click="approveCourseResults"
                                onclick="confirm('Approve these results and forward them to the Exam Officer?') || event.stopImmediatePropagation()"
                    ...
                    @if(!$inspectingCourse)
                        ...
                        <div>
                            ...
                            <select wire:model="statusFilter" class="text-sm border border-gray-300 rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-fuchsia-400">
                                ...
                                <option value="pending">Pending Lecturer Entry</option>
                            </select>
                        </div>
                    @endif
                </div>

                {{-- Summary Metrics Cards (Only on Course List) --}}
                @if(!$inspectingCourse)
                    <div class="px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <span class="text-xs font-semibold text-slate-500 uppercase">Total Department Courses</span>
                            <h4 class="text-2xl font-bold text-slate-800 mt-1">{{ $totalCoursesCount }}</h4>
                        </div>
                        <div class="p-4 bg-amber-50 rounded-xl border border-amber-200">
                            <span class="text-xs font-semibold text-amber-700 uppercase">Awaiting HOD Approval</span>
                            <h4 class="text-2xl font-bold text-amber-800 mt-1">{{ $submittedCoursesCount }}</h4>
                        </div>
                        <div class="p-4 bg-blue-50 rounded-xl border border-blue-200">
                            <span class="text-xs font-semibold text-blue-700 uppercase">HOD Approved (With Exam Officer)</span>
                            <h4 class="text-2xl font-bold text-blue-800 mt-1">{{ $approvedCoursesCount }}</h4>
                        </div>
                        <div class="p-4 bg-green-50 rounded-xl border border-green-200">
                            <span class="text-xs font-semibold text-green-700 uppercase">Released to Students</span>
                            <h4 class="text-2xl font-bold text-green-800 mt-1">{{ $releasedCoursesCount }}</h4>
                        </div>
                    </div>
                @endif

                {{-- Main Content: Course List OR Student Inspection Sheet --}}
                            ...
                            <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                                <thead class="align-bottom">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Assigned Lecturer</th>
                                        <th class="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Enrolled</th>
                                ...
                                <tbody>
                                    @forelse($departmentCourses as $course)
                                        <tr class="hover:bg-slate-50 transition border-b">
                                            ...
                                            <td class="p-4 text-center align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                                <span class="px-2 py-1 text-xs font-bold rounded-lg bg-slate-100 text-slate-700">
                                                    {{ $course->total_registered }} student(s)
                                                </span>
                                            </td>
                                            <td class="p-4 text-center align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                                <div class="flex items-center justify-center gap-1.5 flex-wrap">
                                                    @if($course->submitted_count > 0)
                                                        <span class="px-2 py-0.5 text-xxs font-bold uppercase rounded bg-amber-100 text-amber-800 animate-pulse">
                                                            {{ $course->submitted_count }} Submitted
                                                        </span>
                                                    @endif
                                                    @if($course->hod_approved_count > 0)
                                                        <span class="px-2 py-0.5 text-xxs font-bold uppercase rounded bg-blue-100 text-blue-800">
                                                            {{ $course->hod_approved_count }} Approved
                                                        </span>
                                                    @endif
                                                    @if($course->released_count > 0)
                                                        <span class="px-2 py-0.5 text-xxs font-bold uppercase rounded bg-green-100 text-green-800">
                                            ...
                                            <td class="p-4 text-center align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                                <button wire:click="inspectCourse({{ $course->id }})"
                                                    class="text-xs font-bold text-white bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded-lg transition shadow">
                                                    🔍 Review Scores
                                                </button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="6" class="p-8 text-center align-middle text-slate-400">
                                                No department courses found for <strong>{{ $selectedSession }}</strong> ({{ ucfirst($selectedSemester) }} Semester).
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                                        ...
                                        <tr class="hover:bg-slate-50 transition border-b">
                                            ...
                                            <td class="p-4 text-center align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                                <span class="text-xxs font-bold uppercase px-2.5 py-1 rounded-full
                                                    @if($row['status'] === 'submitted') bg-amber-100 text-amber-800
                                                    @elseif($row['status'] === 'hod_approved') bg-blue-100 text-blue-800
                                                    @elseif($row['status'] === 'released') bg-green-100 text-green-800
                                                    @else bg-gray-100 text-gray-700 @endif">
                                                    {{ $row['status'] }}
                                                </span>
                                                @if(!empty($row['remarks']))
                                                    <p class="text-xxs text-red-500 italic mt-0.5">{{ $row['remarks'] }}</p>
                                                @endif
                                            </td>
                                        </tr>
                                    @empty
        ...
        <div class="fixed inset-0 z-999 flex items-center justify-center overflow-auto bg-black bg-opacity-50">
            <div class="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 m-4 relative animate-fade-up">
                <div class="flex justify-between items-center pb-3 border-b mb-4">
                    <h5 class="text-lg font-bold text-slate-800">Return Results to Lecturer</h5>
                    <button wire:click="closeRejectModal" class="text-slate-400 hover:text-slate-600 text-xl font-bold">&times;</button>
                </div>
                
                <p class="text-sm text-slate-600 mb-4">
                    Please provide a clear reason for returning these results. The lecturer will be notified and results will be unlocked for editing.
                </p>

                <div class="mb-4">
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Reason / Feedback</label>
                    <textarea wire:model.defer="rejectionReason" rows="4" placeholder="E.g. CA scores for student X are missing, please revise and resubmit..."
                        class="w-full text-sm border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-red-400"></textarea>
                ...
                <div class="flex justify-end gap-3 pt-3 border-t">
                    ...
                    <button wire:click="rejectCourseResults" class="px-4 py-2 text-xs font-bold text-white bg-red-600 hover:bg-red-700 rounded-lg uppercase shadow transition">
                        Confirm Return
                    </button>
                </div>
            </div>
