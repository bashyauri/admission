...
class ManageDegreeApplicants extends Component
{
    use WithPagination;
    ...
    public $selectedAcademicSession = '';
    public $sessions = [];
    ...
    {
        $this->sessions = $this->availableSessions();
        $this->selectedAcademicSession = $this->sessions[0] ?? (string) config('remita.settings.academic_session', '');
    }

    public function updatedSelectedAcademicSession(): void
    {
        $this->resetPage();
        $this->clearSelection();
        $this->exportPath = '';
    ...
    private function getApplicantsQuery()
    {
        ...
        $applicants = $service->getDegreeApplicants($this->selectedAcademicSession);
    ...
    {
        $sessions = \App\Models\ProposedCourse::query()
            ->whereHas('user', function ($query) {
                $query->where('role', \App\Enums\Role::APPLICANT->value)
                    ->where('programme_id', \App\Enums\ProgrammesEnum::Undergraduate->value);
            })
        ...
        $configuredSession = config('remita.settings.academic_session');

        if ($configuredSession && !in_array($configuredSession, $sessions, true)) {
            array_unshift($sessions, $configuredSession);
        }

        return $sessions;
    }

    private function validSelectedApplicantIds(): array
    {
        $allowedIds = (new ApplicantCleanupService())
            ->getDegreeApplicants($this->selectedAcademicSession)
            ->pluck('id')
            ->map(fn ($id) => (string) $id)
            ->all();

        return array_values(array_intersect(
            array_map('strval', $this->selectedApplicants),
            $allowedIds
        ));
    }
    ...
    public function openPreviewModal()
    {
        ...
        $service = new ApplicantCleanupService();
        $this->selectedApplicants = $this->validSelectedApplicantIds();

        if (empty($this->selectedApplicants)) {
            $this->errorMessage = 'Selected applicants are not in the chosen academic session.';
            return;
        }
    ...
    public function render()
    {
        ...
        return view('livewire.admin.manage-degree-applicants', [
