...
class SenateGraduationBroadsheetController extends Controller
{
    ...
    /**
     * Display and print official Senate Graduation Broadsheet (Final Degree Master Sheet).
     *
    ...
    public function print(
        ...
        $data = $this->reportingService->getSenateGraduationBroadsheet($filters);

        return view('reports.senate-graduation-broadsheet', $data);
    }
    ...
    public function exportCsv(
        ...
        return response()->streamDownload(function () use ($graduands, $data, $normalizedSession) {
            ...
            fputcsv($handle, ['OFFICIAL SENATE GRADUATION BROADSHEET & DEGREE CONFERMENT LIST']);
            fputcsv($handle, ['ACADEMIC SESSION:', $normalizedSession]);
            ...
            fputcsv($handle, []);
            fputcsv($handle, ['--- SENATE DEGREE CLASSIFICATION SUMMARY ---']);
            $summary = $data['summary'] ?? [];
            fputcsv($handle, ['Total Candidates Audited', $summary['total_graduands'] ?? 0]);
            fputcsv($handle, ['Officially Cleared Graduands', $summary['cleared_count'] ?? 0]);
            fputcsv($handle, ['First Class Honours', ($summary['first_class_count'] ?? 0) . " ({$summary['first_class_percentage']}%)"]);
            fputcsv($handle, ['Second Class Upper Division', ($summary['second_upper_count'] ?? 0) . " ({$summary['second_upper_percentage']}%)"]);
            fputcsv($handle, ['Second Class Lower Division', ($summary['second_lower_count'] ?? 0) . " ({$summary['second_lower_percentage']}%)"]);
            fputcsv($handle, ['Third Class Honours', ($summary['third_class_count'] ?? 0) . " ({$summary['third_class_percentage']}%)"]);
            fputcsv($handle, ['Pass Degree', ($summary['pass_count'] ?? 0) . " ({$summary['pass_percentage']}%)"]);
            fputcsv($handle, ['Deficient / Unapproved', ($summary['deficient_count'] ?? 0) . " ({$summary['deficient_percentage']}%)"]);
