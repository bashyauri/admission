...
cd /usr/share/nginx/html/soft_ui_dashboard_tall_pro/ # go to project repository on the server
git fetch --all
php8.1 artisan down
...
chown -R www-data:www-data /usr/share/nginx/html/soft_ui_dashboard_tall_pro/ #change here too project repository
php8.1 artisan up
source ~/.nvm/nvm.sh
nvm use 16
npm install
